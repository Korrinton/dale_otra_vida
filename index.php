<?php
include_once("header.php");

?>

<main>
    <section class="intro">
        <div class="container principal">
            <h1>Ayuda a la reducción de tu huella de carbón</h1>
            <p>La web definitiva para buscar, encontrar y gestionar el alquiler o la compra de aparatos electrónicos. Fácil, rápido y seguro.</p>
            <div class="botones_primarios">
                <a href="./formulario.php" class="btn btn-primario">Vende tu movil</a>
                <a href="#" class="btn btn-primario">Tablets disponibles</a>
                <a href="#" class="btn btn-primario">Ofertas para empresas</a>
            </div>
        </div>
    </section>

    <section class="características">
        <div class="container">
            <h2>¿Por qué elegir alquilar antes que comprar?</h2>
            <div class="características-grid">
                <section class="características-tarjetas">
                    <h3>Deja de gastar cada año por un móvil nuevo</h3>
                    <p>No gastes por un móvil que va a durarte menos de dos años, </p>
                </section>
                <section class="características-tarjetas">
                    <h3>Reduce la huella digital</h3>
                    <p>El proceso de crear nuevos móviles conlleva una alta tasa de contaminación, al no comprar un móvil
                        ayudas a reducir tu huella de carbono
                    </p>
                </section>
                <section class="características-tarjetas">
                    <h3>¿Te gusta? cómpratelo</h3>
                    <p>Nuestro alquiler con opción a compra permite que si quieres seguir con el móvil que has alquilado 

                    </p>
                </section>
            </div>
        </div>
    </section>
    <script src="./js/index.js"></script>
</main>

<?php

include_once("footer.php");
?>